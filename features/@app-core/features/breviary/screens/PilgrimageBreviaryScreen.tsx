import { useEffect, useState } from 'react';
import { ScrollView, Text, TouchableOpacity, View } from 'react-native';
import { useDispatch } from 'react-redux';

import { pilgrimageRouteTheme } from '../../../../../packages/@app-ui';
import {
  READING_FONT_SCALE_STEPS,
  ReadingFontSizeControl,
  type ReadingFontScale,
} from '../../../components/ReadingFontSizeControl';
import { AppLoader } from '../../../components/AppLoader';
import {
  brewiarzApi,
  type BreviaryOfficeId,
  useGetBreviaryOfficeQuery,
} from '../../../services/brewiarzApi';
import type { AppDispatch } from '../../../store/store';
import { BreviarySectionCard } from '../components/BreviarySectionCard';
import { OFFICE_OPTIONS } from '../helpers/pilgrimageBreviary.helpers';
import { OfficeTabs } from '../components/OfficeTabs';

const { colors, typography } = pilgrimageRouteTheme;
const BREVIARY_ACCENT = '#2563EB';
const BREVIARY_ACCENT_SOFT = '#EFF6FF';
const BREVIARY_ACCENT_BORDER = '#BFDBFE';

type PilgrimageBreviaryScreenProps = {
  onBack: () => void;
};

export function PilgrimageBreviaryScreen({ onBack }: PilgrimageBreviaryScreenProps) {
  const dispatch = useDispatch<AppDispatch>();
  const [activeOffice, setActiveOffice] = useState<BreviaryOfficeId>('jutrznia');
  const [fontScale, setFontScale] = useState<ReadingFontScale>(READING_FONT_SCALE_STEPS[0]);
  const { data, isLoading, isFetching, isError } = useGetBreviaryOfficeQuery(activeOffice);

  useEffect(() => {
    const subscriptions = OFFICE_OPTIONS.map(({ id }) =>
      dispatch(
        brewiarzApi.endpoints.getBreviaryOffice.initiate(id, {
          subscribe: false,
          forceRefetch: false,
        })
      )
    );

    return () => {
      for (const subscription of subscriptions) {
        subscription.unsubscribe();
      }
    };
  }, [dispatch]);

  return (
    <ScrollView
      className="flex-1"
      style={{ backgroundColor: colors.surface }}
      contentContainerClassName="px-4 pt-6 pb-6"
      showsVerticalScrollIndicator={false}>
      <TouchableOpacity activeOpacity={0.75} onPress={onBack}>
        <Text
          className="mb-5 text-[15px] font-semibold"
          style={{ color: BREVIARY_ACCENT, fontFamily: typography.fontFamily }}>
          Wróć do niezbędnika
        </Text>
      </TouchableOpacity>

      <Text
        className="text-[28px] font-bold"
        style={{ color: colors.onSurface, fontFamily: typography.fontFamily }}>
        Brewiarz
      </Text>
      <Text
        className="mt-2 text-[16px] leading-6"
        style={{
          color: colors.onSurfaceVariant,
          fontFamily: typography.fontFamily,
          fontSize: 16,
          lineHeight: 24,
        }}>
        Dzisiejsza Liturgia Godzin z serwisu brewiarz.pl.
      </Text>

      <OfficeTabs value={activeOffice} onChange={setActiveOffice} />
      <ReadingFontSizeControl
        value={fontScale}
        onChange={setFontScale}
        accentColor={BREVIARY_ACCENT}
        borderColor={BREVIARY_ACCENT_BORDER}
      />

      {isLoading || isFetching ? (
        <View className="mt-8">
          <AppLoader label="Pobieranie tekstow brewiarza..." fontScale={fontScale} minHeight={144} />
        </View>
      ) : isError || !data || data.sections.length === 0 ? (
        <Text
          className="mt-8 text-[16px] leading-7"
          style={{
            color: '#9b3d3d',
            fontFamily: typography.fontFamily,
            fontSize: 16 * fontScale,
            lineHeight: 28 * fontScale,
          }}>
          Nie udalo sie pobrac tekstu wybranej godziny brewiarza.
        </Text>
      ) : (
        <View className="mt-8 gap-4">
          <View
            className="rounded-[24px] border px-5 py-5"
            style={{
              borderColor: BREVIARY_ACCENT_BORDER,
              borderRadius: 24,
              backgroundColor: BREVIARY_ACCENT_SOFT,
            }}>
            <Text
              className="text-[13px] font-semibold uppercase tracking-[1px]"
              style={{ color: BREVIARY_ACCENT, fontFamily: typography.fontFamily }}>
              Dzisiejsze oficjum
            </Text>
            <Text
              className="mt-1 text-[21px] font-medium"
              style={{ color: colors.onSurface, fontFamily: typography.fontFamily }}>
              {data.officeLabel}
            </Text>
            <Text
              className="mt-2 text-[15px] leading-6"
              style={{
                color: colors.onSurfaceVariant,
                fontFamily: typography.fontFamily,
                fontSize: 15 * fontScale,
                lineHeight: 24 * fontScale,
              }}>
              {data.date}
            </Text>
            {data.psalterWeek ? (
              <Text
                className="mt-2 text-[15px] leading-6"
                style={{
                  color: colors.onSurfaceVariant,
                  fontFamily: typography.fontFamily,
                  fontSize: 15 * fontScale,
                  lineHeight: 24 * fontScale,
                }}>
                {data.psalterWeek}
              </Text>
            ) : null}
            {data.season ? (
              <Text
                className="mt-1 text-[15px] leading-6"
                style={{
                  color: colors.onSurfaceVariant,
                  fontFamily: typography.fontFamily,
                  fontSize: 15 * fontScale,
                  lineHeight: 24 * fontScale,
                }}>
                {data.season}
              </Text>
            ) : null}
            {data.liturgicalDay ? (
              <Text
                className="mt-1 text-[15px] leading-6"
                style={{
                  color: colors.onSurfaceVariant,
                  fontFamily: typography.fontFamily,
                  fontSize: 15 * fontScale,
                  lineHeight: 24 * fontScale,
                }}>
                {data.liturgicalDay}
              </Text>
            ) : null}
          </View>

          {data.sections.map((section) => (
            <BreviarySectionCard
              key={`${data.office}-${section.id}`}
              {...section}
              fontScale={fontScale}
            />
          ))}

          <Text
            className="px-1 text-[13px] leading-5"
            style={{
              color: colors.onSurfaceVariant,
              fontFamily: typography.fontFamily,
              fontSize: 13 * fontScale,
              lineHeight: 20 * fontScale,
            }}>
            Zrodlo: {data.sourceUrl}
          </Text>
        </View>
      )}
    </ScrollView>
  );
}
